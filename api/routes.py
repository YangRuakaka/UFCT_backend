...
from data import HFDatasetFetcher
...
# 全局对象
fetcher: Optional[HFDatasetFetcher] = None
...
def get_fetcher():
    """获取或创建 HFDatasetFetcher 实例"""
    global fetcher
    if fetcher is None:
        try:
            logger.info("初始化 Hugging Face Dataset Fetcher...")
            fetcher = HFDatasetFetcher()
            logger.info("✓ Fetcher 初始化成功")
        except Exception as e:
            logger.error(f"✗ Fetcher 初始化失败: {e}")
            raise
    return fetcher
...
def get_citation_network():
    ...
    parameters:
      ...
      - name: layout
        ...
        required: false
        default: spring
        enum: [spring, kamada_kawai, circular]
        description: 布局算法
    responses:
      200:
...
# ==================== 论文详情 ====================
@api_bp.route('/papers/<string:paper_id>', methods=['GET'])
...
# ==================== 作者详情 ====================
@api_bp.route('/authors/<string:author_id>', methods=['GET'])
def get_author_details(author_id: str):
    ...
    try:
        ...
        # 获取作者详情
        fetcher_instance = get_fetcher()
        author_data = fetcher_instance.get_author_details(author_id)
        ...
        # 缓存结果
        cache.set(cache_key, author_data)
